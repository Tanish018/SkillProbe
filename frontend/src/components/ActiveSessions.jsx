      ...
      <div className="card-body">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <div className="size-2 bg-success rounded-xl " />
            <span className="text-sm font-medium text-success">{sessions.length} Active</span>
          </div>
        </div>

        {/* Sessions List */}
        <div className="space-y-3 max-h-100 overflow-y-auto py-2">
          {isLoading ? (
            ...
            sessions.map((session) => (
              <div key={session._id} className="card bg-base-200 border-base-300 hover:border-primary/50">
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className={`badge badge-sm font-medium ${getDifficultyBadgeClass(session.difficulty)}`}>{session.difficulty.slice(0, 1).toUpperCase() + session.difficulty.slice(1)}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-sm opacity-80">
                      ...
                      {session.participant && !isUserInSession(session) ? (
                        <span className="badge badge-error badge-sm font-semibold">FULL</span>
                      ) : (
                        <span className="badge badge-success badge-sm font-semibold">OPEN</span>
                      )}
                    </div>
                  </div>
